...
describe('registerService', () => {
  ...
  it('sends a welcome email on successful registration', async () => {
    ...
    mockRepo.create.mockResolvedValueOnce('507f1f77bcf86cd799439011');
    await registerService({ fullName: 'Bob', email: 'bob@example.com', password: 'password123' });
    expect(mockMailer.sendWelcomeEmail).toHaveBeenCalledWith('bob@example.com', 'Bob');
  });

  it('does not send a welcome email when the account already exists', async () => {
    mockRepo.findByEmail.mockResolvedValueOnce(fakeUser as never);
    await registerService({ fullName: 'Alice', email: 'alice@example.com', password: 'password123' });
    expect(mockMailer.sendWelcomeEmail).not.toHaveBeenCalled();
  });
});
