...
```
STATUS: locked
Product:    Consumer app — one-day tour booking platform
Palette:    Deep & moody + electric indigo accent (dark-luxury/glassmorphism default)
            base #FAFAFA (light, toggle only) / #0A0A0A (dark, default); accent #5B5BFF
            tokens: --primary oklch(0.555 0.243 271) light / oklch(0.62 0.21 271) dark
            glass surfaces: translucent card bg + backdrop-blur + hairline white/10 border
...
Feel:       Balanced — CSS load-rise stagger + hover micro-interactions; reduced-motion respected;
            glass cards over a soft indigo glow behind the hero
Signature:  Dark full-bleed hero with glass badge + one accent word + honest stat strip;
            featured-tours glass card grid pulling live data from the tours collection;
            dashboard app-shell (admin) stays on the same tokens, mobile drawer sidebar
```
